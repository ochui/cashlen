...
class LogClass extends ErrorAction
{
    ...
    protected function renderAjaxResponse()
    {
        if($this->error_id!=null) {
            ...
            if($this->debug==false)
                return json_encode(
                    array("status" => "error",
                        "message" => "An error has occured, we have been informed about the same. Use reference id: <strong>" . $this->error_id . "</strong> to address this case with our support team.",
            ...
            if($this->debug==false)
                return json_encode(
                    array("status" => "error",
                        "message" => "An error has occured, we have been informed about the same. "
                    )
                );
            else{
                return json_encode(array("status"=>"error","message"=>$this->getExceptionName() . ': ' . $this->getExceptionMessage()));
    ...
    protected function renderHtmlResponse()
    {
        ...
        if($this->debug == true)
            ...
            if($this->error_id!=null)
                return "An error has occured, we have been informed about the same. Use reference id: <strong>".$this->error_id."</strong> to address this case with our support team.";
            else
                return "An error has occured, we have been informed about the same. ";
        }
    }
